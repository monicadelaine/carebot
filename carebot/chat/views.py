                ...
                if is_sql:
                    try:
                        ...
                        try:
                            completion = client.chat.completions.create(
                                model="gpt-3.5-turbo",
                                messages=[{"role": "system", "content": f"Explain the following results of a PostgreSQL query as if you are telling the user about healthcare resources you found in Alabama. Each {resource_type} found in {city} is in the following format:\nagency name, address part 1, address part 2, city, id_cms_other (ignore this), resource type\nresults: {response_text}"}],
                            )
                            ai_response = completion.choices[0].message.content
                        except Exception as e:
